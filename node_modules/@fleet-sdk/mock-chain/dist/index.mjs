import { ensureDefaults, Network, utxoSum, assert, isUndefined, some, first, decimalize, Collection, isDefined, ensureUTxOBigInt } from '@fleet-sdk/common';
import { hex, blake2b256, randomBytes, bigintBE, utf8 } from '@fleet-sdk/crypto';
import { AvlTree$, GroupElement$, ProverBuilder$ } from 'sigmastate-js/main';
import { SAFE_MIN_BOX_VALUE, ErgoAddress, DuplicateInputError, NotFoundError } from '@fleet-sdk/core';
import { serializeBox, decode } from '@fleet-sdk/serializer';
...
function log(value, part) {
  const colored = part.added ? pc.green(`+ ${value}`) : part.removed ? pc.red(`- ${value}`) : pc.gray(`  ${value}`);
  console.log(colored);
}
...
var MockUTxOCollection = class extends Collection {
  constructor(boxes) {
    super();
    if (isDefined(boxes)) {
      this.add(boxes);
...
var MockChain = class {
  ...
  newParty(optOrName) {
    return this.#pushParty(
      typeof optOrName === "string" || isUndefined(optOrName) ? new KeyedMockChainParty(this, optOrName) : new NonKeyedMockChainParty(this, optOrName.ergoTree, optOrName.name)
    );
  }
  ...
  execute(unsignedTransaction, options, baseCost) {
    ...
    if (!result.success) {
      if (options?.log) {
        log2(pc.red(`${pc.bgRed(pc.bold(" Error "))} ${result.reason}`));
      }
...
var safeUtf8Encode = (v) => v instanceof Uint8Array ? utf8.encode(v) : void 0;

export { BLOCKCHAIN_PARAMETERS, KeyedMockChainParty, MockChain, MockChainParty, NonKeyedMockChainParty, execute, mockBlockchainStateContext, mockHeaders, mockUTxO };
